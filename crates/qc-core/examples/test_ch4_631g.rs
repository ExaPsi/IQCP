// Test CH4 6-31G integrals and SCF
...
fn main() {
    ...
    // Try without DIIS first
    println!("--- Without DIIS ---");
    let mut config = ScfConfig::new(ConvergenceProfile::Medium);
    ...
    match rhf_scf(&system, &config) {
        Ok(result) => {
            ...
            for iter in &result.trace {
                if let (Some(de), Some(rms)) = (iter.delta_e, iter.rms_density_change) {
                    println!(
                        "  Iter {}: E={:.10}, dE={:.2e}, RMS={:.2e}",
                        iter.iteration, iter.energy_total, de, rms
                    );
                } else {
                    println!(
                        "  Iter {}: E={:.10} (initial)",
                        iter.iteration, iter.energy_total
                    );
                }
            }
        }
        Err(e) => {
            ...
            match rhf_scf(&system, &config) {
                ...
                Err(e) => {
                    println!("SCF with DIIS also failed: {}", e);
                }
            }
        }
    }
